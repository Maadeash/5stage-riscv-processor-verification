#!/usr/bin/env python3
"""
run_coverage_regression.py

Automates: compile (with coverage) -> run full regression -> report
for the RV32I UVM testbench on Synopsys VCS.

Usage:
    python3 run_coverage_regression.py

Requires: filelist.f in the current directory (same file you already pass
to vcs with -f), and $UVM_HOME set OR -ntb_opts uvm-1.2 working on your
VCS install.

IMPORTANT: all test runs share ONE -cm_dir (CM_DIR below). Per-test
"-cm_dir <unique>" was tried and abandoned -- it strips the compile-time
elaborated design out of each test's coverage snapshot, so urg cannot
properly merge them (you get "Design not yet loaded" / "Limited design
loaded" and a broken, partial merged report). Keeping every run in the
same -cm_dir (tagged only by -cm_name) is what actually works with this
VCS/urg version.
"""
...
# ---------------------------------------------------------------------------
# Config -- edit this list if tests are added/removed/renamed
# ---------------------------------------------------------------------------
TESTS = [
...
FILELIST = "filelist.f"
CM_DIR = "simv.vdb"          # ONE shared coverage database for every run
LOG_DIR = "logs"
...
def parse_dashboard_table(text):
    """
    Real urg layout (X-2025.06-SP2-1) is a header row of metric names
    followed immediately by one row of values, e.g.:
        Total Coverage Summary
        SCORE  LINE   COND   TOGGLE FSM    BRANCH ASSERT GROUP
         21.72  25.23  25.48   7.75 --      14.86  27.71  29.30
    Returns a list of (metric, value) tuples, or [] if the pattern isn't found.
    """
    m = re.search(
...
# ---------------------------------------------------------------------------
# 2. Run every test into the SAME shared coverage database
# ---------------------------------------------------------------------------
def run_regression():
...
# ---------------------------------------------------------------------------
# 3. Generate the human-readable report directly from the shared database
#    (no merge step -- every run above already accumulated into CM_DIR)
# ---------------------------------------------------------------------------
def generate_report():
...
# ---------------------------------------------------------------------------
# 4. Overall code + functional (group) coverage percentages
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Per-test FUNCTIONAL coverage -- pulled directly from each test's own
#    already-printed [COV] lines (rv32i_coverage.sv prints these itself).
#    This sidesteps urg per-test isolation entirely, which is fragile with
#    this VCS setup -- the data below has been reliably correct all along.
# ---------------------------------------------------------------------------
def parse_per_test_functional_coverage():
    ...
    for t in TESTS:
        ...
        # keep the LAST occurrence of each category (final cumulative value
        # printed at end of test, if a test prints more than once)
        seen = {}
