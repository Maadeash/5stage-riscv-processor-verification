    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if (obs.regwrite_wb && (obs.rd_wb != 5'd0) && vif.monitor_cb.memwb_valid) begin
                `uvm_info("MON",
                          $sformatf("[%0d] WB: x%0d = 0x%08h (src=%0d)",
                                    cycle_count, obs.rd_wb, obs.rd_val_wb, obs.wb_src),
                          UVM_HIGH)
                ap_wb.write(obs);
            end
            ...
            if (vif.monitor_cb.exmem_valid &&
                ...
                `uvm_info("MON",
                          $sformatf("[%0d] MEM: %s addr=0x%08h data=0x%08h f3=%03b",
                                    cycle_count,
                                    vif.monitor_cb.exmem_memwrite ? "STORE" : "LOAD",
                                    obs.mem_addr, obs.mem_wdata, obs.mem_funct3),
                          UVM_HIGH)
                ap_mem.write(obs);
            end
            ...
            if (obs.trap_detected) begin
                `uvm_info("MON",
                          $sformatf("[%0d] TRAP: cause=0x%08h",
                                    cycle_count, obs.trap_cause_obs),
                          UVM_MEDIUM)
                ap_trap.write(obs);
            end
        end
